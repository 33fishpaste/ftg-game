...
class BattleSystem {
    ...
    updateCommandButtons(player, prefix) {
        const available = this.getAvailableCommands(player);
        COMMAND_LIST.forEach(cmd => {
            const id = `${prefix}-${this.cmdId(cmd)}`;
    ...
    updateUI(p1, p2) {
        document.getElementById('p1HpBar').style.width = `${p1.hp}%`;
        document.getElementById('p2HpBar').style.width = `${p2.hp}%`;
        document.getElementById('p1SidestepCD').textContent = p1.sidestep_cd;
        document.getElementById('p2SidestepCD').textContent = p2.sidestep_cd;
        document.getElementById('p1HoldCounterCD').textContent = p1.hold_counter_cd;
        document.getElementById('p2HoldCounterCD').textContent = p2.hold_counter_cd;
        document.getElementById('p1Trauma').textContent = p1.trauma;
        document.getElementById('p2Trauma').textContent = p2.trauma;
        this.updateCommandButtons(p1, 'p1');
        this.updateCommandButtons(p2, 'p2');
    }
    ...
    async startBattle() {
        ...
        const name1 = document.getElementById('player1NameLabel');
        const name2 = document.getElementById('player2NameLabel');
...
function loadBattleNames() {
    const name1 = document.getElementById('player1NameLabel');
    const name2 = document.getElementById('player2NameLabel');
    if (name1) name1.textContent = localStorage.getItem('p1Name') || 'Player 1';
    if (name2) name2.textContent = localStorage.getItem('p2Name') || 'Player 2';
}
